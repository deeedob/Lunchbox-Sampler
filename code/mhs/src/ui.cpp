...
mhs::UI::UI( uint8_t w, uint8_t h, TwoWire *twi, int8_t rst_pin,
             u_int32_t preclk, uint32_t postclk) : Window(w, h, twi, rst_pin, preclk, postclk,2) {
    Wire1.begin();
...
void mhs::UI::drawMenu() {
    ...
    //float val = static_cast<float>(analogRead(POT0)) / 1024.f;
    //Positions p = this->splitHorizontal(2, 0.5f, 4, 2,
