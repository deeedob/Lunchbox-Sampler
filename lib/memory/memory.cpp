...
bool Memory::deleteSamplePack( const char* path ) {
	if( !SD.begin( C_SDCARD_CS_PIN )) {
		Serial.println( "Unable to access SPI Flash chip" );
		return false;
	}
...
bool Memory::loadSample( const char* path ) {
	if( !SD.begin( C_SDCARD_CS_PIN )) {
		Serial.println( "Unable to access SPI Flash chip" );
		return false;
	}
...
SerialFlashFile Memory::getSample( const char* path ) {
	if( !SerialFlash.begin( FLASH_PIN )) {
...
bool Memory::replaceSample( const char* oldPath , const char* newPath ) {
	
	return true;
}

bool Memory::deleteSample( const char* name ) {
	if( !SD.begin( C_SDCARD_CS_PIN )) {
		Serial.println( "Unable to access SPI Flash chip" );
		return false;
	}
...
bool Memory::compare( File& file , SerialFlashFile& ffile ) {
	file.seek( 0 );
	ffile.seek( 0 );
...
void Memory::setSettingsFile( String* settings ) {
	return;
}