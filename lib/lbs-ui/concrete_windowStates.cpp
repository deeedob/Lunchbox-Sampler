...
void WindowFullscreen::draw( Window* win... ) {
	...
	std::pair< std::pair< u_int8_t , u_int8_t > , std::pair< u_int8_t , u_int8_t>> activeWindow = win->configs
	                                                                                                 ->getActiveWindow();
	...
	for( int i = 0; i < iterations; i++ ) {
		...
		win->drawRoundRect( x0 , y0 , x1 , y1 , roundness , static_cast<uint16_t>(WindowConfigs::CLRS::CLR_ACTIVE)); // only one window to draw it active
		_p += globPad;
	}
	this->setWindowArea( {{ x0 , y0 } ,
	                      { x1 , y1 }} );
...
bool WindowFullscreen::equals( const AbstractWindowState& b ) {
	return dynamic_cast<const WindowFullscreen*>(&b) != nullptr;
}
...
void WindowVertSplit::draw( Window* win... ) {
	...
	for( int i = 0; i < iterations; i++ ) {
		...
		win->drawRoundRect( win1x0 , win1y0 , win1Width , win1Height , roundness , static_cast<uint8_t>(WindowConfigs::getActiveWinClrs()
			.first)); // winUp
		
		win->drawRoundRect( win2x0 , win2y0 , win2Width , win2Height , roundness , static_cast<uint8_t>(WindowConfigs::getActiveWinClrs()
			.second)); // winDown
	...
	this->setWindowAreaUp( {{ win1x0 ,             win1y0 } ,
	                        { win1x0 + win1Width , win1y0 + win1Height }} );
	this->setWindowAreaDown( {{ win2x0 ,             win2y0 } ,
	                          { win2x0 + win2Width , win2y0 + win2Height }} );
	win->display(); // render to screen
}
...
bool WindowVertSplit::equals( const AbstractWindowState& b ) {
	if( dynamic_cast<const WindowVertSplit*>(&b)) return true;
	return false;
}
...
void WindowHorizSplit::draw( Window* win... ) {
	...
	this->setWindowAreaLeft( {{ win1x0 ,             win1y0 } ,
	                          { win1x0 + win1Width , win1y0 + win1Height }} );
	this->setWindowAreaRight( {{ win2x0 ,             win2y0 } ,
	                           { win2x0 + win2Width , win2y0 + win2Height }} );
	win->display(); // render to screen
}
