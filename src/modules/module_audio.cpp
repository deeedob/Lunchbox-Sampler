...
using namespace lbs;
ModuleAudio::ModuleAudio() : AbstractModule( "Audio" ) {
...
void ModuleAudio::update( Graphics* g, Events::DIGITAL e ) {
	...
	if( m_currentMainState == m_mainStates.begin() + 2 ) {
		if( m_currentEffect == m_effects.begin() + 1 ) {
			if( e == Events::DIGITAL::ROTARY_R ) {
				if( m_currentDetailFilter == m_detailFilter.begin() ) { m_currentDetailFilter = m_detailFilter.end(); }
				m_currentDetailFilter--;
				draw_audio_module_sub2( g );
				return;
			}
			if( e == Events::DIGITAL::ROTARY_L ) {
				m_currentDetailFilter++;
				if( m_currentDetailFilter == m_detailFilter.end() ) { m_currentDetailFilter = m_detailFilter.begin(); }
				draw_audio_module_sub2( g );
				return;
			}
			if( e == Events::DIGITAL::BTN_RETURN ) {
			...
			if( e == Events::DIGITAL::BTN_ENTER ) {
				if( m_currentDetailFilter == m_detailFilter.begin() ) {
					highpass = !highpass;
					draw_audio_module_sub2( g );
					return;
				}
				if( m_currentDetailFilter == m_detailFilter.begin() + 1 ) {
					m_currentMainState++;
					draw_audio_module_sub2( g );
					return;
				}
		...
		if( m_currentEffect == m_effects.begin() + 2 ) {
			if( e == Events::DIGITAL::ROTARY_R ) {
				m_currentDetailDelay++;
				if( m_currentDetailDelay == m_detailDelay.end() ) { m_currentDetailDelay = m_detailDelay.begin(); }
				draw_audio_module_sub2( g );
				return;
			}
			if( e == Events::DIGITAL::ROTARY_L) {
				if( m_currentDetailDelay == m_detailDelay.begin() ) { m_currentDetailDelay = m_detailDelay.end(); }
				m_currentDetailDelay--;
				draw_audio_module_sub2( g );
				return;
			}
		...
		if( m_currentEffect == m_effects.begin() + 3 ) {
			if( e == Events::DIGITAL::ROTARY_R ) {
				m_currentDetailCompressor++;
				if( m_currentDetailCompressor == m_detailCompressor.end() ) { m_currentDetailCompressor = m_detailCompressor.begin(); }
				draw_audio_module_sub2( g );
				return;
			}
			if( e == Events::DIGITAL::ROTARY_L) {
				if( m_currentDetailCompressor == m_detailCompressor .begin() ) { m_currentDetailCompressor = m_detailCompressor.end(); }
				m_currentDetailCompressor--;
				draw_audio_module_sub2( g );
				return;
			}
	...
	if( m_currentMainState == m_mainStates.begin() + 3 ) {
		if( m_currentEffect == m_effects.begin() + 1 ) {
			...
			if( e == Events::DIGITAL::BTN_ENTER ||  e == Events::DIGITAL::BTN_RETURN)
			{
				m_currentMainState--;
				draw_audio_module_sub2( g );
				return;
...
void ModuleAudio::exit() { notify( *this ); }